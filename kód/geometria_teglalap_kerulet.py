...
def teglalapkerulet(masterwindow):
    def calc(*args):
        a = alengthscan.get()
        b = blengthscan.get()
        res = 2 * (int(a)+int(b))
    ...
    teglak = Toplevel(masterwindow, height=400, width=600)
    teglak.title("Python SPAR projekt")
    teglak.minsize(width = 600, height = 400)
    teglak.maxsize(width = 600, height = 400)
    selection = IntVar()
    selection.set(1)
    alengthscan = StringVar()
    blengthscan = StringVar()
    menusor = Frame(teglak, width=600, height=400, bd=10, highlightbackground="green", highlightthickness=10)
    ...
    videos.add_command(label = "Kilépés", command = teglak.destroy, underline = 0,background="#EC4949")
    menu6.config(menu = videos)
    ize2=Label(teglak, text="Sponsored by SPAR ™", fg="green",font=('Silkscreen', 8))
    ize2.place(x=30,y=350)
    can1 = Canvas(teglak, width=10, height=10)
    photo = PhotoImage(file='python-project\képek\spar-logo-1.png')
    item = can1.create_image(20, 20, image = photo)
    can1.place(x=150,y=350)

    textbox = Label(teglak, text="Téglalap Kerület", fg="green")
    a = Label(teglak, text="a", fg="green")
    alength = Entry(teglak, textvariable=alengthscan, width=35)
    alength.focus()
    b = Label(teglak, text="b", fg="green")
    blength = Entry(teglak, textvariable=blengthscan, width=35)
    alengthscan.trace_add("write", calc)
    blengthscan.trace_add("write", calc)
    results = Entry(teglak, width=35)
    resulttext = Label(teglak, text="Eredmény:", fg="green")
